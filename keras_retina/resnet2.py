...
from keras_frcnn.resnet50 import ResNet
def get_weight_path():
...
def classifier_layers(x, input_shape, trainable=False):
    ...
    if K.backend() == 'tensorflow':
        x = conv_block_td(x, 3, [512, 512, 2048], stage=6, block='a', input_shape=input_shape, strides=(2, 2), trainable=trainable)
    elif K.backend() == 'theano':
        x = conv_block_td(x, 3, [512, 512, 2048], stage=6, block='a', input_shape=input_shape, strides=(1, 1), trainable=trainable)

    x = identity_block_td(x, 3, [512, 512, 2048], stage=6, block='b', trainable=trainable)
    x = identity_block_td(x, 3, [512, 512, 2048], stage=6, block='c', trainable=trainable)
